...
calculate_parallel_gauss (struct calculation_arguments const* arguments, struct calculation_results* results, struct options const* options)
{
	...
	while (term_iteration > 0)
	{
		...
		Wir verwenden Irecv (nichtblockierend), da die letzte Zeile nicht gleich zu Anfang benötigt wird.*/
		if((it_count != 0) && (rang != arguments->comm_size - 1))
        {
            MPI_Irecv((void*) &Matrix_In[arguments->rows - 1][0], N + 1, MPI_DOUBLE, rang + 1, 1, MPI_COMM_WORLD, &request_recv_l);
        }
        ...
        Des weiteren empfangen wir das maxResiduum des vorherigen Prozesses. Hierfür könnten wir streng genommen auch Irecv verwenden, allerdings
        werden die Zeile und das Residuum gleichzeitig versendet, es ist also zu erwarten dass sie auch gleichzeitig empfangen werden können.*/
		if(rang != 0)
        {
            MPI_Recv((void*) &Matrix_In[0][0], N + 1, MPI_DOUBLE, rang - 1, 1, MPI_COMM_WORLD, &status_recv);
            MPI_Recv((void*) &global_res, 1, MPI_DOUBLE, rang - 1, 2, MPI_COMM_WORLD, &status_recv);
        }
        ...
        for (i = 1; (uint64_t)i < arguments->rows - 1; i++)
		{
			...
			/*  SENDEN DER ZWEITEN ZEILE mit Isend
			Alle Prozesse außer dem ersten senden ihre erste Zeile an den vorherigen Prozess.
            ...
            {
                /*senden von M[1]*/
                MPI_Isend((void*) &Matrix_Out[1][0], N + 1, MPI_DOUBLE, rang - 1, 1, MPI_COMM_WORLD, &request_send_f);
            }
            /*  ABSCHLUSS SENDEN DER VORLETZTEN ZEILE mit Isend
            Alle Prozesse außer dem letzten schließen das Senden ihrer vorletzten Zeile ab.
            In Iteration 0 wurde noch nicht versendet, weshalb in diesem Fall nicht gewartet werden muss.
            Fall (i == arguments->rows - 2) gilt verändern wir als nächstes die vorletzte Zeile, weshalb das Versenden vorher abgeschlossen werden muss.*/
            else if((it_count != 0) && (rang != arguments->comm_size - 1) && ((uint64_t)i == arguments->rows - 2))
            {
                MPI_Wait(&request_send_l, &status_send_l);
            }
            /*Alle Prozesse außer dem letzten schließen das Versenden ihres maxResiduums ab, bevor dieses verändert wird.*/
            else if((it_count != 0) && (rang != arguments->comm_size - 1) && (i == 1))
            {
                MPI_Wait(&request_send_r, &status_send_l);
            }

            /*  ABSCHLUSS: EMPFANGEN LETZTER ZEILE mit Irecv
            Alle Prozesse außer dem letzten müssen ihre neue letzte Zeile empfangen, bevor sie die Berechnung ihrer vorletzten Zeile starten.
            Dies ist der Fall, wenn (i == arguments->rows - 2) gilt.
            Beim ersten Durchlauf der Berechnung (vor dem Abschluss der ersten Iteration) hat noch keine Kommunikation stattgefunden,
            weshalb hier nicht gewartet werden muss (it_count != 0).*/
            if((it_count != 0) && (rang != arguments->comm_size - 1) && ((uint64_t)i == arguments->rows - 2))
            {
                MPI_Wait(&request_recv_l, &status_send_l);
            }
		...
		global_res = (maxResiduum > global_res ? maxResiduum : global_res);

        /*  SENDEN DER VORLETZTEN ZEILE mit Isend
            SENDEN DES MAXRESIDUUM mit Isend
        ...
        (entweder ihr eigenes, oder dass Residuum das ihnen zugesendet wurde)
        Wann genau versendet wird ist diesem Prozess egal, weshalb wir Isend verwenden.*/
        if(rang != arguments->comm_size - 1)
        {
            MPI_Isend((void*) &global_res, 1, MPI_DOUBLE, rang + 1, 2, MPI_COMM_WORLD, &request_send_r);

            MPI_Isend((void*) &Matrix_Out[arguments->rows - 2][0], N + 1, MPI_DOUBLE, rang + 1, 1, MPI_COMM_WORLD, &request_send_l);
        }
		...
		if (options->termination == TERM_PREC)
		{
		    ...
		    Falls dies der Fall ist, sendet er eine (leere) Nachricht an Prozess 0.*/
		    if(rang == arguments->comm_size - 1 && !flag_sent)
            {
                if(global_res < options->term_precision)
                {
                    MPI_Send((void*) &i, 1, MPI_INT, 0, 3, MPI_COMM_WORLD);

                    /*Wir setzen die Flag auf true*/
                    flag_sent = 1;
                }
            }

            /*Dieser prüft ob der letzte Prozess ihm eine Nachricht gesendet hat.
            Dies wird mit Iprobe geprüft um einen Deadlock zu vermeiden.*/
            if(rang == 0)
            {
                MPI_Iprobe(arguments->comm_size - 1, 3, MPI_COMM_WORLD, &flag_precision, &status_recv);

                /*Falls der letzte Prozess eine Nachricht versendet hat, sendet Prozess 0 das Abbruchsignal (NULL) an den folgenden Prozess
                und verlässt die while-Schleife durch (term_iteration = 0).*/
                if(flag_precision)
                {
                    term_iteration = 0;

                    MPI_Send(NULL, 0, MPI_INT, rang + 1, 3, MPI_COMM_WORLD);
                }
            }
            else
            {
                /*Jeder Prozess überprüft ob der vorherige Prozess das Abbruchsignal gegeben hat.
                Dies reicht aus, da sich durch den Austausch der ersten und letzten Zeile alle Prozesse
                in einer Iteration befinden, die sich höchstens um 1 unterscheidet.*/
                MPI_Iprobe(rang - 1, 3, MPI_COMM_WORLD, &flag_precision, &status_recv);

                /*Falls dies der Fall ist, wird die while-Schleife verlassen und dem nächsten Prozess das Signal übermittelt.*/
                if(flag_precision)
                {
                    term_iteration = 0;
	...
	/*  SENDEN DER VORLETZTEN ZEILE mit Isend
	Für die letzte Iteration muss das letzte Versenden abgeschlossen werden.*/
	if(rang != arguments->comm_size - 1)
    {
        MPI_Wait(&request_send_l, &status_send_l);
    }
